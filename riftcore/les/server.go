...
import (
	...
	"github.com/cryptorift/riftcore/les/flowcontrol"
...
func NewLesServer(rift *rift.CryptoRift, config *rift.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(rift.BlockChain().Config(), false, config.NetworkId, rift.EventMux(), rift.Engine(), newPeerSet(), rift.BlockChain(), rift.TxPool(), rift.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	pm.blockLoop()
	...
	srv := &LesServer{
		protocolManager: pm,
		quitSync:        quitSync,
		lesTopic:        lesTopic(rift.BlockChain().Genesis().Hash()),
	}
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(rift.ChainDb())
	return srv, nil
}
...
func newCostStats(db riftdb.Database) *requestCostStats {
...
func getChtRoot(db riftdb.Database, num uint64) common.Hash {
...
func storeChtRoot(db riftdb.Database, num uint64, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], num)
	db.Put(append(chtPrefix, encNumber[:]...), root[:])
}

func makeCht(db riftdb.Database) bool {
