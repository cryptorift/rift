...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [rift/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [rift/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [rift/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [rift/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [rift/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [rift/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [rift/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [rift/63] Channel receiving inbound node state data
...
func New(mode SyncMode, stateDb riftdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
...
// these are zero.
func (d *Downloader) Progress() cryptorift.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "rift", p.version, "head", hash, "td", td, "mode", d.mode)
