...
import (
	...
	"github.com/cryptorift/riftcore/trie"
)

const defaultTraceTimeout = 5 * time.Second

// PublicCryptoriftAPI provides an API to access CryptoRift full node-related
// information.
type PublicCryptoriftAPI struct {
	e *CryptoRift
}

// NewPublicCryptoriftAPI creates a new Cryptorift protocol API for full nodes.
func NewPublicCryptoriftAPI(e *CryptoRift) *PublicCryptoriftAPI {
	return &PublicCryptoriftAPI{e}
}

// Riftbase is the address that mining rewards will be send to
func (api *PublicCryptoriftAPI) Riftbase() (common.Address, error) {
	return api.e.Riftbase()
}

// Coinbase is the address that mining rewards will be send to (alias for Riftbase)
func (api *PublicCryptoriftAPI) Coinbase() (common.Address, error) {
	return api.Riftbase()
}

// Hashrate returns the POW hashrate
func (api *PublicCryptoriftAPI) Hashrate() hexutil.Uint64 {
...
func NewPublicMinerAPI(e *CryptoRift) *PublicMinerAPI {
...
func NewPrivateMinerAPI(e *CryptoRift) *PrivateMinerAPI {
...
// SetRiftbase sets the riftbase of the miner
func (api *PrivateMinerAPI) SetRiftbase(riftbase common.Address) bool {
	api.e.SetRiftbase(riftbase)
	return true
}
...
// PrivateAdminAPI is the collection of Cryptorift full node-related APIs
...
// admin methods of the CryptoRift service.
func NewPrivateAdminAPI(rift *CryptoRift) *PrivateAdminAPI {
	return &PrivateAdminAPI{rift: rift}
}
...
func (api *PrivateAdminAPI) ExportChain(file string) (bool, error) {
	...
	if err := api.rift.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.rift.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.rift.BlockChain().InsertChain(blocks); err != nil {
...
// PublicDebugAPI is the collection of Cryptorift full node APIs exposed
...
// related public debug methods of the CryptoRift service.
func NewPublicDebugAPI(rift *CryptoRift) *PublicDebugAPI {
	return &PublicDebugAPI{rift: rift}
}
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	if blockNr == rpc.PendingBlockNumber {
		...
		_, stateDb := api.rift.miner.Pending()
		return stateDb.RawDump(), nil
	}
	var block *types.Block
	if blockNr == rpc.LatestBlockNumber {
		block = api.rift.blockchain.CurrentBlock()
	} else {
		block = api.rift.blockchain.GetBlockByNumber(uint64(blockNr))
	}
	if block == nil {
		return state.Dump{}, fmt.Errorf("block #%d not found", blockNr)
	}
	stateDb, err := api.rift.BlockChain().StateAt(block.Root())
	if err != nil {
...
// PrivateDebugAPI is the collection of Cryptorift full node APIs exposed over
...
// private debug methods of the CryptoRift service.
func NewPrivateDebugAPI(config *params.ChainConfig, rift *CryptoRift) *PrivateDebugAPI {
	return &PrivateDebugAPI{config: config, rift: rift}
}
...
type BlockTraceResult struct {
	...
	StructLogs []riftapi.StructLogRes `json:"structLogs"`
...
func (api *PrivateDebugAPI) TraceBlock(blockRlp []byte, config *vm.LogConfig) BlockTraceResult {
	...
	return BlockTraceResult{
		Validated:  validated,
		StructLogs: riftapi.FormatLogs(logs),
...
func (api *PrivateDebugAPI) TraceBlockByNumber(blockNr rpc.BlockNumber, config *vm.LogConfig) BlockTraceResult {
	...
	case rpc.PendingBlockNumber:
		...
		block = api.rift.miner.PendingBlock()
...
func (api *PrivateDebugAPI) TraceBlockByHash(hash common.Hash, config *vm.LogConfig) BlockTraceResult {
	// Fetch the block that we aim to reprocess
	block := api.rift.BlockChain().GetBlockByHash(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) traceBlock(block *types.Block, logConfig *vm.LogConfig) (bool, []vm.StructLog, error) {
	...
	var (
		blockchain = api.rift.BlockChain()
	...
	if err := api.rift.engine.VerifyHeader(blockchain, block.Header(), true); err != nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, txHash common.Hash, config *TraceArgs) (interface{}, error) {
	...
	if config != nil && config.Tracer != nil {
		...
		var err error
		if tracer, err = riftapi.NewJavascriptTracer(*config.Tracer); err != nil {
			return nil, err
		}
		...
		go func() {
			<-deadlineCtx.Done()
			tracer.(*riftapi.JavascriptTracer).Stop(&timeoutError{})
		}()
		defer cancel()
	} else if config == nil {
	...
	tx, blockHash, _, txIndex := core.GetTransaction(api.rift.ChainDb(), txHash)
	if tx == nil {
	...
	case *vm.StructLogger:
		return &riftapi.ExecutionResult{
			Gas:         gas,
			ReturnValue: fmt.Sprintf("%x", ret),
			StructLogs:  riftapi.FormatLogs(tracer.StructLogs()),
		}, nil
	case *riftapi.JavascriptTracer:
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int) (core.Message, vm.Context, *state.StateDB, error) {
	...
	block := api.rift.BlockChain().GetBlockByHash(blockHash)
	if block == nil {
		return nil, vm.Context{}, nil, fmt.Errorf("block %x not found", blockHash)
	}
	parent := api.rift.BlockChain().GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
		return nil, vm.Context{}, nil, fmt.Errorf("block parent %x not found", block.ParentHash())
	}
	statedb, err := api.rift.BlockChain().StateAt(parent.Root())
	if err != nil {
	...
	for idx, tx := range txs {
		...
		context := core.NewEVMContext(msg, block.Header(), api.rift.BlockChain(), nil)
		if idx == txIndex {
...
func (api *PrivateDebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	db := core.PreimageTable(api.rift.ChainDb())
...
func (api *PrivateDebugAPI) GetBadBlocks(ctx context.Context) ([]core.BadBlockArgs, error) {
	return api.rift.BlockChain().BadBlocks()
}
