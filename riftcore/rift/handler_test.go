...
func testGetNodeData(t *testing.T, protocol int) {
	...
	hashes := []common.Hash{}
	for _, key := range pm.chaindb.(*riftdb.MemDatabase).Keys() {
	...
	statedb, _ := riftdb.NewMemDatabase()
...
// Tests that post rift protocol handshake, DAO fork-enabled clients also execute
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	peer, _ := newTestPeer("peer", rift63, pm, true)
	defer peer.close()
