...
*/
/** @file io.h
 * @author Lefteris Karapetsas <lefteris@cryptoriftdev.com>
 * @date 2015
 */
#pragma once
#include <stdlib.h>
...
// Reference: https://github.com/cryptorift/wiki/wiki/Rifthash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see rifthash_io_prepare
enum rifthash_io_rc {
	RIFTHASH_IO_FAIL = 0,           ///< There has been an IO failure
	RIFTHASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	RIFTHASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	RIFTHASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of rifthash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * rifthash_full_t
 */
#ifdef RIFTHASH_PRINT_CRITICAL_OUTPUT
#define RIFTHASH_CRITICAL(...)							\
	do													\
	{													\
		printf("RIFTHASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for rifthash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the rifthash
 ...
 *                           https://github.com/cryptorift/wiki/wiki/Rifthash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum rifthash_io_rc
 */
enum rifthash_io_rc rifthash_io_prepare(
	char const* dirname,
	rifthash_h256_t const seedhash,
	FILE** output_file,
	uint64_t file_size,
	bool force_create
);
 ...
 */
FILE* rifthash_fopen(char const* file_name, char const* mode);
 ...
 *                         error returns NULL
 */
char* rifthash_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool rifthash_mkdir(char const* dirname);
 ...
 */
bool rifthash_file_size(FILE* f, size_t* ret_size);
 ...
 */
char* rifthash_io_create_filename(
	char const* dirname,
	char const* filename,
 ...
 *
 * The spec defining this directory is here: https://github.com/cryptorift/wiki/wiki/Rifthash-DAG
 *
 ...
 */
bool rifthash_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool rifthash_io_mutable_name(
	uint32_t revision,
