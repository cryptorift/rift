...
// Package riftcmd contains the simplified mobile APIs to riftcore.
//
// The scope of this package is *not* to allow writing a custom CryptoRift client
// with pieces plucked from riftcore, rather to allow writing native dapps on
...
// from the riftcore repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original riftcore version.
