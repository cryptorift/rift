...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Riftcmd.LightScryptN, Riftcmd.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.cryptorift", new NodeConfig());
			node.start();
			...
			CryptoRiftClient ec = node.getCryptoriftClient();
	...
	// signatures too. Regression test for riftcore issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "riftcmd-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/cryptorift/riftcmdtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Riftcmd and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.cryptorift", "github.com/cryptorift/riftcore/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "riftcmd.aar"), "riftcmd.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "cryptorift", "riftcmdtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.cryptorift.riftcmdtest"
