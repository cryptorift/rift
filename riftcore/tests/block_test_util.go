...
// Package tests implements execution of CryptoRift JSON tests.
package tests
...
func (t *BlockTest) Run(config *params.ChainConfig) error {
	...
	chain, err := core.NewBlockChain(db, config, rifthash.NewShared(), new(event.TypeMux), vm.Config{})
	if err != nil {
		return err
	}
	defer chain.Stop()
...
/* See https://github.com/cryptorift/tests/wiki/Blockchain-Tests-II
