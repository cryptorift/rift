...
// Package riftstats implements the network stats reporting service.
package riftstats
...
import (
	...
	"github.com/cryptorift/riftcore/common/mclock"
...
// Service implements an CryptoRift netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	stack *node.Node // Temporary workaround, remove when API finalized

	server *p2p.Server        // Peer-to-peer server to retrieve networking infos
	rift    *rift.CryptoRift      // Full CryptoRift service if monitoring a full node
	les    *les.LightCryptorift // Light CryptoRift service if monitoring a light node
...
func New(url string, riftServ *rift.CryptoRift, lesServ *les.LightCryptorift) (*Service, error) {
	...
	for {
		...
		if len(msg["emit"]) == 2 && command == "history" {
			...
			if !ok {
				...
				s.histCh <- nil
				continue // Riftstats sometime sends invalid history requests, ignore those
			}
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["rift"]; info != nil {
		network = fmt.Sprintf("%d", info.(*rift.RiftNodeInfo).Network)
		protocol = fmt.Sprintf("rift/%d", rift.ProtocolVersions[0])
	} else {
		network = fmt.Sprintf("%d", infos.Protocols["les"].(*rift.RiftNodeInfo).Network)
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the riftstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to riftstats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to riftstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.rift != nil {
		...
		if block == nil {
			block = s.rift.BlockChain().CurrentBlock()
		}
		header = block.Header()
		td = s.rift.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(list) > 0 {
		...
		var head int64
		if s.rift != nil {
			head = s.rift.BlockChain().CurrentHeader().Number.Int64()
		} else {
	...
	for i, number := range indexes {
		...
		if s.rift != nil {
			block = s.rift.BlockChain().GetBlockByNumber(number)
		} else {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to riftstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	var pending int
	if s.rift != nil {
		pending, _ = s.rift.TxPool().Stats()
	} else {
		pending = s.les.TxPool().Stats()
	}
	// Assemble the transaction stats and send it to the server
	log.Trace("Sending pending transactions to riftstats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.rift != nil {
		mining = s.rift.Miner().Mining()
		hashrate = int(s.rift.Miner().HashRate())

		sync := s.rift.Downloader().Progress()
		syncing = s.rift.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock

		price, _ := s.rift.ApiBackend.SuggestPrice(context.Background())
	...
	log.Trace("Sending node details to riftstats")
