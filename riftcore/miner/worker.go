...
func newWorker(config *params.ChainConfig, engine consensus.Engine, coinbase common.Address, rift Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		config:         config,
		engine:         engine,
		rift:            rift,
		mux:            mux,
		chainDb:        rift.ChainDb(),
		recv:           make(chan *Result, resultQueueSize),
		chain:          rift.BlockChain(),
		proc:           rift.BlockChain().Validator(),
		possibleUncles: make(map[common.Hash]*types.Block),
		coinbase:       coinbase,
		txQueue:        make(map[common.Hash]*types.Transaction),
		agents:         make(map[Agent]struct{}),
		unconfirmed:    newUnconfirmedBlocks(rift.BlockChain(), 5),
...
func (self *worker) setRiftbase(addr common.Address) {
	self.mu.Lock()
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	wallets := self.rift.AccountManager().Wallets()
...
func (self *worker) commitNewWork() {
	...
	pending, err := self.rift.TxPool().Pending()
	if err != nil {
	...
	self.rift.TxPool().RemoveBatch(work.failedTxs)
