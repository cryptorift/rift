...
// Package miner implements CryptoRift block creation and mining.
package miner
...
func New(rift Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		rift:      rift,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, rift, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(rift.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.setRiftbase(coinbase)
...
func (self *Miner) SetRiftbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setRiftbase(addr)
}
