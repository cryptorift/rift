...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (riftdb.Database, error) {
	if ctx.config.DataDir == "" {
		return riftdb.NewMemDatabase()
	}
	db, err := riftdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
