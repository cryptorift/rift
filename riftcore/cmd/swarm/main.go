...
import (
	...
	"github.com/cryptorift/riftcore/contracts/ens"
	...
	"github.com/cryptorift/riftcore/swarm"
	bzzapi "github.com/cryptorift/riftcore/swarm/api"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:  "swap-api",
		Usage: "URL of the CryptoRift API provider to use to settle SWAP payments",
	}
	...
	EnsAPIFlag = cli.StringFlag{
		Name:  "ens-api",
		Usage: "URL of the CryptoRift API provider to use for ENS record lookups",
		Value: node.DefaultIPCEndpoint("riftcmd"),
	}
	...
	DeprecatedRiftAPIFlag = cli.StringFlag{
		Name:  "riftapi",
...
// This init function sets defaults so cmd/swarm can run alongside riftcmd.
func init() {
...
var app = utils.NewApp(gitCommit, "CryptoRift Swarm")

// This init function creates the cli.App.
func init() {
	app.Action = bzzd
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2016 The CryptoRift Authors"
...
func bzzd(ctx *cli.Context) error {
	// exit if the deprecated --riftapi flag is set
	if ctx.GlobalString(DeprecatedRiftAPIFlag.Name) != "" {
		utils.Fatalf("--riftapi is no longer a valid command line flag, please use --ens-api and/or --swap-api.")
	}
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := riftclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
...
func registerBzzService(ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		...
		if ensapi != "" {
			...
			ensClient = riftclient.NewClient(client)
