...
// faucet is a Rifter faucet backed by a light client.
package main
...
var (
	...
	riftPortFlag = flag.Int("riftport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the CryptoRift protocol")
	statsFlag   = flag.String("riftstats", "", "Riftstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Rifters to pay out per user request")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for CryptoRift and the faucet")
)

var (
	rifter = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Rifters", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *riftPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account  common.Address     `json:"account"`  // CryptoRift address being funded
	Time     time.Time          `json:"time"`     // Timestamp when te request was accepted
	Tx       *types.Transaction `json:"tx"`       // Transaction funding the account
}

// faucet represents a crypto faucet backed by an CryptoRift light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // CryptoRift protocol stack
	client *riftclient.Client   // Client connection to the CryptoRift chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the CryptoRift light client protocol
	...
	// Assemble the riftstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightCryptorift
			ctx.Service(&serv)
			return riftstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := riftclient.NewClient(api)
...
// close terminates the CryptoRift connection and tears down the faucet.
...
// apiHandler handles requests for Rifter grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	websocket.JSON.Send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, rifter),
		"funded":   nonce,
	...
	for {
		...
		// Iterate over all the files and look for CryptoRift addresses
		...
		if address == (common.Address{}) {
			websocket.JSON.Send(conn, map[string]string{"error": "No CryptoRift address found to fund"})
			continue
		}
		...
		if timeout = f.timeouts[gist.Owner.Login]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), rifter)
...
func (f *faucet) loop() {
	...
	for {
		...
		case head := <-heads:
			...
			balance, _ := f.client.BalanceAt(context.Background(), f.account.Address, nil)
			balance = new(big.Int).Div(balance, rifter)
