...
func TestAccountListEmpty(t *testing.T) {
	riftcmd := runRiftcmd(t, "account", "list")
	riftcmd.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	riftcmd := runRiftcmd(t, "account", "list", "--datadir", datadir)
	defer riftcmd.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	riftcmd := runRiftcmd(t, "account", "new", "--lightkdf")
	defer riftcmd.ExpectExit()
	riftcmd.Expect(`
...
`)
	riftcmd.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	riftcmd := runRiftcmd(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	riftcmd := runRiftcmd(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer riftcmd.ExpectExit()
	riftcmd.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(riftcmd.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(riftcmd.StderrText(), m) {
...
// https://github.com/cryptorift/riftcore/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	riftcmd.SetTemplateFunc("keypath", func(file string) string {
