...
type config struct {
	...
	riftstats  string        // Riftstats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // CryptoRift services known to be running on servers
...
// converts it to an CryptoRift address. If an empty line is entered, the default
// value is returned.
