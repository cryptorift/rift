...
func (w *wizard) networkStats(tips bool) {
	...
	for server, pubkey := range w.conf.Servers {
		...
		logger.Debug("Checking for riftstats availability")
		if infos, err := checkRiftstats(client, w.network); err != nil {
			if err != ErrServiceUnknown {
				services["riftstats"] = err.Error()
			}
		} else {
			services["riftstats"] = infos.String()
			protips.riftstats = infos.config
		}
	...
	if protips.riftstats != "" {
		w.conf.riftstats = protips.riftstats
	}
...
func (p *protips) print(network string) {
	...
	fullinit, lightinit := "", ""
	if p.genesis != "" {
		fullinit = fmt.Sprintf("riftcmd --datadir=$HOME/.%s init %s.json && ", network, network)
		lightinit = fmt.Sprintf("riftcmd --datadir=$HOME/.%s --light init %s.json && ", network, network)
	}
	// If an riftstats server is available, add the riftstats flag
	statsflag := ""
	if p.riftstats != "" {
		if strings.Contains(p.riftstats, " ") {
			statsflag = fmt.Sprintf(` --riftstats="yournode:%s"`, p.riftstats)
		} else {
			statsflag = fmt.Sprintf(` --riftstats=yournode:%s`, p.riftstats)
		}
	}
	...
	tips = append(tips, fmt.Sprintf("%sriftcmd --networkid=%d --datadir=$HOME/.%s --cache=1024%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a full node with recent data only")
	tips = append(tips, fmt.Sprintf("%sriftcmd --networkid=%d --datadir=$HOME/.%s --cache=512 --fast%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a light node with on demand retrievals")
	tips = append(tips, fmt.Sprintf("%sriftcmd --networkid=%d --datadir=$HOME/.%s --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))

	tasks = append(tasks, "Run an embedded node with constrained memory")
	tips = append(tips, fmt.Sprintf("%sriftcmd --networkid=%d --datadir=$HOME/.%s --cache=32 --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))
