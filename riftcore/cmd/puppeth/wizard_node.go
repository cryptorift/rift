...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.riftstats == "" {
		log.Error("No riftstats server configured")
		return
	}
	...
	if infos.riftstats == "" {
		...
		infos.riftstats = w.readString() + ":" + w.conf.riftstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.riftstats)
		infos.riftstats = w.readDefaultString(infos.riftstats) + ":" + w.conf.riftstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.genesis.Config.Rifthash != nil {
			// Rifthash based miners only need an riftbase to mine against
			fmt.Println()
			if infos.riftbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.riftbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.riftbase)
				infos.riftbase = w.readDefaultAddress(common.HexToAddress(infos.riftbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootFull, w.conf.bootLight, infos); err != nil {
		log.Error("Failed to deploy CryptoRift node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
