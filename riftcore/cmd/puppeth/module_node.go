...
// nodeDockerfile is the Dockerfile required to run an CryptoRift node.
var nodeDockerfile = `
FROM cryptorift/client-go:alpine-develop
...
{{end}}
RUN \
  echo '/riftcmd init /genesis.json' > riftcmd.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.cryptorift/keystore/ && cp /signer.json /root/.cryptorift/keystore/' >> riftcmd.sh && \{{end}}
	echo $'/riftcmd --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --riftstats \'{{.Riftstats}}\' {{if .BootV4}}--bootnodesv4 {{.BootV4}}{{end}} {{if .BootV5}}--bootnodesv5 {{.BootV5}}{{end}} {{if .Riftbase}}--riftbase {{.Riftbase}} --mine{{end}}{{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> riftcmd.sh

ENTRYPOINT ["/bin/sh", "riftcmd.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an CryptoRift node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    volumes:
      - {{.Datadir}}:/root/.cryptorift
    environment:
      - FULL_PORT={{.FullPort}}/tcp
      - LIGHT_PORT={{.LightPort}}/udp
      - TOTAL_PEERS={{.TotalPeers}}
      - LIGHT_PEERS={{.LightPeers}}
      - STATS_NAME={{.Riftstats}}
      - MINER_NAME={{.Riftbase}}
...
func deployNode(client *sshClient, network string, bootv4, bootv5 []string, config *nodeInfos) ([]byte, error) {
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Riftstats":   config.riftstats[:strings.Index(config.riftstats, ":")],
		"Riftbase":  config.riftbase,
...
func (info *nodeInfos) String() string {
	...
	return fmt.Sprintf("port=%d%s, datadir=%s, peers=%d, lights=%d, riftstats=%s, gastarget=%0.3f MGas, gasprice=%0.3f GWei",
		info.portFull, discv5, info.datadir, info.peersTotal, info.peersLight, info.riftstats, info.gasTarget, info.gasPrice)
}
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 /riftcmd --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.cryptorift"],
		portFull:   infos.portmap[infos.envvars["FULL_PORT"]],
		portLight:  infos.portmap[infos.envvars["LIGHT_PORT"]],
		peersTotal: totalPeers,
		peersLight: lightPeers,
		riftstats:   infos.envvars["STATS_NAME"],
		riftbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasPrice:   gasPrice,
