...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"riftstats", "explorer", "wallet", "faucet"} {
	...
	// If we have riftstats running, ask whether to make the secret public or not
	var riftstats bool
	if w.conf.riftstats != "" {
		fmt.Println()
		fmt.Println("Include riftstats secret on dashboard (y/n)? (default = yes)")
		riftstats = w.readDefaultString("y") == "y"
	}
	// Try to deploy the dashboard container on the host
	if out, err := deployDashboard(client, w.network, infos.port, infos.host, listing, &w.conf, riftstats); err != nil {
