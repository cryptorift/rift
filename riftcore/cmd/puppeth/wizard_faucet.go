...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Rifters to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.riftstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.riftstats = w.readString() + ":" + w.conf.riftstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.riftstats)
		infos.node.riftstats = w.readDefaultString(infos.node.riftstats) + ":" + w.conf.riftstats
	}
