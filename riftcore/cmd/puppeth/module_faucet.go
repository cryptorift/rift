...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	...
	(cd $GOPATH/src/github.com/cryptorift && git clone --depth=1 https://github.com/cryptorift/riftcore) && \
  go build -v github.com/cryptorift/riftcore/cmd/faucet                                              && \
...
CMD [ \
	"/faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--riftstats", "{{.Riftstats}}", "--riftport", "{{.RiftPort}}", \
...
services:
  faucet:
    ...
    ports:
      - "{{.RiftPort}}:{{.RiftPort}}"{{if not .VHost}}
      - "{{.ApiPort}}:8080"{{end}}
    volumes:
      - {{.Datadir}}:/root/.faucet
    environment:
      - RIFT_PORT={{.RiftPort}}
      - RIFT_NAME={{.RiftNameame}}
...
func deployFaucet(client *sshClient, network string, bootnodes []string, config *faucetInfos) ([]byte, error) {
	...
	template.Must(template.New("").Parse(faucetDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Riftstats":      config.node.riftstats,
		"RiftPort":       config.node.portFull,
	...
	template.Must(template.New("").Parse(faucetComposefile)).Execute(composefile, map[string]interface{}{
		...
		"ApiPort":       config.port,
		"RiftPort":       config.node.portFull,
		"RiftNameame":       config.node.riftstats[:strings.Index(config.node.riftstats, ":")],
		"GitHubUser":    config.githubUser,
		"GitHubToken":   config.githubToken,
		"CaptchaToken":  config.captchaToken,
		"CaptchaSecret": config.captchaSecret,
...
func (info *faucetInfos) String() string {
	return fmt.Sprintf("host=%s, api=%d, rift=%d, amount=%d, minutes=%d, tiers=%d, github=%s, captcha=%v, riftstats=%s", info.host, info.port, info.node.portFull, info.amount, info.minutes, info.tiers, info.githubUser, info.captchaToken != "", info.node.riftstats)
}
...
func checkFaucet(client *sshClient, network string) (*faucetInfos, error) {
	...
	return &faucetInfos{
		node: &nodeInfos{
			...
			portFull: infos.portmap[infos.envvars["RIFT_PORT"]+"/tcp"],
			riftstats: infos.envvars["RIFT_NAME"],
			keyJSON:  keyJSON,
			keyPass:  keyPass,
		},
		host:          host,
