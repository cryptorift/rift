...
// PublicCryptoriftAPI provides an API to access CryptoRift related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicCryptoriftAPI struct {
	b Backend
}

// NewPublicCryptoriftAPI creates a new Cryptorift protocol API.
func NewPublicCryptoriftAPI(b Backend) *PublicCryptoriftAPI {
	return &PublicCryptoriftAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicCryptoriftAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current CryptoRift protocol version this node supports
func (s *PublicCryptoriftAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicCryptoriftAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19Cryptorift Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Cryptorift Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an CryptoRift ECDSA signature for:
// keccack256("\x19Cryptorift Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/cryptorift/riftcore/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with rift_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Cryptorift Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/cryptorift/riftcore/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid CryptoRift signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the CryptoRift blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Cryptorift blockchain API.
...
//
// https://github.com/cryptorift/wiki/wiki/JSON-RPC#rift_sign
...
// PublicDebugAPI is the collection of Cryptorift APIs exposed over the public
// debugging endpoint.
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("0x%x", rifthash.SeedHash(number)), nil
}

// PrivateDebugAPI is the collection of Cryptorift APIs exposed over the private
// debugging endpoint.
...
// Version returns the current cryptorift protocol version.
