...
type Config struct {
	Oracle common.Address // CryptoRift address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the CryptoRift service dependency to access the blockchain
	var apiBackend riftapi.Backend
	var cryptorift *rift.CryptoRift
	if err := ctx.Service(&cryptorift); err == nil {
		apiBackend = cryptorift.ApiBackend
	} else {
		var cryptorift *les.LightCryptorift
		if err := ctx.Service(&cryptorift); err == nil {
			apiBackend = cryptorift.ApiBackend
		} else {
			return nil, err
	...
	contract, err := NewReleaseOracle(config.Oracle, rift.NewContractBackend(apiBackend))
	if err != nil {
		return nil, err
	}
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/cryptorift/riftcore/releases for new releases")
