...
import (
	...
	"github.com/cryptorift/riftcore/internal/jsre"
	"github.com/cryptorift/riftcore/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute CryptoRift requests through
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var rift = web3.rift; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = rift.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines rift.getBalance(rift.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Riftcmd instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Riftcmd metadata
	fmt.Fprintf(c.printer, "Welcome to the Riftcmd JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + rift.coinbase);
		console.log("at block: " + rift.blockNumber + " (" + new Date(1000 * rift.getBlock(rift.blockNumber).timestamp) + ")");
