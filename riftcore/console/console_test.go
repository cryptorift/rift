...
func newTester(t *testing.T, confOverride func(*rift.Config)) *tester {
	...
	// Create a networkless protocol stack and start an CryptoRift service within
	stack, err := node.New(&node.Config{DataDir: workspace, UseLightweightKDF: true, Name: testInstance})
	if err != nil {
		t.Fatalf("failed to create node: %v", err)
	}
	riftConf := &rift.Config{
		Genesis:   core.DevGenesisBlock(),
		Riftbase: common.HexToAddress(testAddress),
		PowTest:   true,
	}
	if confOverride != nil {
		confOverride(riftConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return rift.New(ctx, riftConf) }); err != nil {
		t.Fatalf("failed to register CryptoRift protocol: %v", err)
	}
	...
	stack.Service(&cryptorift)

	return &tester{
		workspace: workspace,
		stack:     stack,
		cryptorift:  cryptorift,
		console:   console,
		input:     prompter,
		output:    printer,
	}
