...
#
# The cluster contains a bootnode, a riftcmd node and multiple swarm nodes, with
...
# static riftcmd configuration
RIFTCMD_IP="192.168.33.3"
RIFTCMD_RPC_PORT="8545"
RIFTCMD_RPC_URL="http://${RIFTCMD_IP}:${RIFTCMD_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the riftcmd and
# swarm nodes
start_bootnode() {
...
# start_riftcmd_node starts a riftcmd node with --datadir pointing at <base-dir>/riftcmd
# and a single, unlocked account with password "riftcmd"
start_riftcmd_node() {
  local dir="${base_dir}/riftcmd"
  mkdir -p "${dir}"

  local password="riftcmd"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating riftcmd account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get riftcmd account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${RIFTCMD_IP}"
    --rpcport   "${RIFTCMD_RPC_PORT}"
    --verbosity "6"
  )

  start_node "riftcmd" "${RIFTCMD_IP}" "$(which riftcmd)" ${args[@]}
}
...
create_account() {
  ...
  riftcmd --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
