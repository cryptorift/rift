...
type testOdr struct {
	OdrBackend
	sdb, ldb riftdb.Database
	disable  bool
}

func (odr *testOdr) Database() riftdb.Database {
	return odr.ldb
}
...
type odrTestFn func(ctx context.Context, db riftdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error)

func TestOdrGetBlockLes1(t *testing.T) { testChainOdr(t, 1, odrGetBlock) }

func odrGetBlock(ctx context.Context, db riftdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrGetReceipts(ctx context.Context, db riftdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrAccounts(ctx context.Context, db riftdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrContractCall(ctx context.Context, db riftdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func testChainGen(i int, block *core.BlockGen) {
	...
	switch i {
	case 0:
		// In block 1, the test bank sends account #1 some rifter.
		...
		block.AddTx(tx)
	case 1:
		// In block 2, the test bank sends some more rifter to account #1.
...
func testChainOdr(t *testing.T, protocol int, fn odrTestFn) {
	var (
		...
		sdb, _  = riftdb.NewMemDatabase()
		ldb, _  = riftdb.NewMemDatabase()
		gspec   = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis = gspec.MustCommit(sdb)
	)
	gspec.MustCommit(ldb)
	// Assemble the test environment
	blockchain, _ := core.NewBlockChain(sdb, params.TestChainConfig, rifthash.NewFullFaker(), evmux, vm.Config{})
	...
	lightchain, err := NewLightChain(odr, params.TestChainConfig, rifthash.NewFullFaker(), evmux)
	if err != nil {
		t.Fatal(err)
	}
